...
feature 'create encouragement'do
  ...
  let(:user2) { FactoryGirl.create(:user, email: 'email123@email.pl') }
  let(:achievement) { FactoryGirl.create(:achievement, user: user2) }
  ...
  scenario 'authenticated user leaves encouragement for achievement'do
    ...
    expect(page).to have_css('#encouragament_quantity', text: '1')
